...
export default {
  ...
  actions: {
    async fetchsiswa({ commit }) {
      try {
        const datasiswa = await axios.get(
          "https://tabungan-siswa-api.000webhostapp.com/api/user-siswa",
          {
            headers: {
    ...
    async fetchsiswaadd(context, student) {
      ...
      try {
        const response = await axios.post(
          "https://tabungan-siswa-api.000webhostapp.com/api/siswa/add",
          student,
          {
    ...
    async fetchsiswaid(context, id) {
      try {
        const response = await axios.get(
          `https://tabungan-siswa-api.000webhostapp.com/api/siswa/show/${id}`,
          {
            headers: {
              "Content-Type": "application/json",
            },
    ...
    async fecteditsiswa({ commit, getters }, data) {
      try {
        const response = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/siswa/edit/${getters.getsiswaid.id}`,
          data,
          {
            headers: { "content-type": "application/x-www-form-urlencoded" },
          }
    ...
    async fectsiswadelete({ commit }, id) {
      try {
        const datasiswa = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/siswa/delete/${id}`
        );
