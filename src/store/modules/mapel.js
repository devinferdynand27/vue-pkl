...
export default {
  ...
  actions: {
    async fectmapel({ commit }) {
      try {
        const darimapel = await axios.get(
          "https://tabungan-siswa-api.000webhostapp.com/api/mapel"
        );
    ...
    async fetchmapeladd({ commit }, data) {
      try {
        const darimapel = await axios.post(
          "https://tabungan-siswa-api.000webhostapp.com/api/mapel/add",
          data,
          {
    ...
    async fetchmapeledit({ commit, getters }, data) {
      try {
        const darimapel = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/mapel/edit/${getters.getmapelsingle.id}`,
          data,
          {
            headers: { "content-type": "application/x-www-form-urlencoded" },
          }
    ...
    async fectmapelshow({ commit }, id) {
      try {
        const darimapel = await axios.get(
          `https://tabungan-siswa-api.000webhostapp.com/api/mapel/show/${id}`
        );
    ...
    async fectmapeldelete({ commit }, id) {
      try {
        const datakelas = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/mapel/delete/${id}`
        );
