...
export default {
  ...
  actions: {
    async fecttugas({ commit }) {
      try {
        const datatugas = await axios.get(
          "https://tabungan-siswa-api.000webhostapp.com/api/tugas-siswa"
        );
    ...
    async fetchtugasadd(context, data) {
      try {
        const response = await axios.post(
          "https://tabungan-siswa-api.000webhostapp.com/api/tugas-siswa/add",
          data,
          {
    ...
    async fetchtugasid({ commit }, id) {
      try {
        const datatuugas = await axios.get(
          `https://tabungan-siswa-api.000webhostapp.com/api/tugas-siswa/show/${id}`
        );
    ...
    async fectedittugas({ commit, getters }, data) {
      try {
        const response = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/tugas-siswa/edit/${getters.gettugassingle.id}`,
          data,
          {
            headers: { "content-type": "application/x-www-form-urlencoded" },
          }
    ...
    async fectdeletetugassiswa({ commit }, id) {
      try {
        const response = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/tugas-siswa/delete/${id}`
        );
        commit("SET_TUGAS", response.data.data);
      } catch (error) {
        console.error(error);
