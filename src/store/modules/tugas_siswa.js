...
export default {
  ...
  actions: {
    async fetchtugasiswa({ commit }) {
      try {
        const datasiswa = await axios.get(
          "https://tabungan-siswa-api.000webhostapp.com/api/user/tugas-pengumpulan"
        );
    ...
    async fecthtugassiswadetail({ commit }, id) {
      try {
        const datasiswa = await axios.get(
          `https://tabungan-siswa-api.000webhostapp.com/api/user/tugas/detail/${id}`
        );
    ...
    async kumpulkan_tugas({ commit ,getters }, data ) {
      try {
        const kumpulkan_tugass = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/user/pengerjaan-tugas/add/${getters.getsingle_siswa.id}`,
          data,
          {
