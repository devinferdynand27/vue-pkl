...
export default {
  ...
  actions: {
    async fectkelas({ commit }) {
      try {
        const datakelas = await axios.get(
          "https://tabungan-siswa-api.000webhostapp.com/api/kelas"
        );
    ...
    async fetchkelasadd(context, student) {
      try {
        const response = await axios.post(
          "https://tabungan-siswa-api.000webhostapp.com/api/kelas/add",
          student,
          {
    ...
    async fetchkelasshow({ commit }, id) {
      try {
        const datasiswa = await axios.get(
          `https://tabungan-siswa-api.000webhostapp.com/api/kelas/show/${id}`
        );
    ...
    async fetchkelasedit({ commit, getters }, data) {
      try {
        const response = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/kelas/edit/${getters.getkelasid.id}`,
          data,
          {
            headers: { "content-type": "application/x-www-form-urlencoded" },
          }
    ...
    async fectkelasdelete({ commit }, id) {
      try {
        const datakelas = await axios.post(
          `https://tabungan-siswa-api.000webhostapp.com/api/kelas/delete/${id}`
        );
